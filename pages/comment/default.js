...
Page({
  ...
  whenStopRecordAudio: function (res) {
    ...
    wx.uploadFile({
      ...
      success(res) {
        //服务器端保存音频后，返回一个文件上传临时ID，用于用户确认提交后关联至该评论
  ...
  confirmPostComment() {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          that.requetTheAnswer()
        }
      }
    })
  },
  // 提交问题的request请求部分
  requetTheAnswer: function () {
    var that = this
    wx.request({
      url: getApp().globalData.requestUrl + 'ashx/getPostDataComment.ashx',
      method: 'post',
      header: {
      ...
      success: function (result) {
        ...
        setTimeout(function () {
          var navurl = ''
          if (that.data.commenttype == '4'){//语培
            navurl = '../cultivate/details?culid='
            wx.removeStorageSync('culdetaildatatime_' + that.data.commentid)
            wx.removeStorageSync('culdetaildata_' + that.data.commentid)
          }
          if (that.data.commenttype == '3') {//顾问
            navurl = '../institution/details?insid='
            wx.removeStorageSync('insdetailtime_' + that.data.commentid)
            wx.removeStorageSync('insdetaildata_' + that.data.commentid)
          }
          wx.navigateTo({
            url: navurl + that.data.commentid,
          })
        }, 800)//间隔0.8秒
      }
    })
  ...
  onLoad: function (options) {
    ...
    that.setData({
      commenttype: options.objtype,
      commentid: options.objid,
    })
  },
