...
Page({
  ...
  bindPickerChange(e) {
    this.setData({
      ...
      pickervalue: this.data.pickeobjrarray[e.detail.value].id
    })
  },
  inputContactInfo(e){
    if(e.detail.value != ''){
  ...
  inputQaTitle(e) {
    if (e.detail.value != '') {
      this.setData({ qatitle: e.detail.value })
    }
  },
  inputQaContent(e) {
    if (e.detail.value != '') {
  ...
  confirmPostQuestion(formId){
    var that = this
    if (that.data.qacontent != '' && that.data.qatitle != ''){
      wx.showModal({
        title: '提交问题',
        content: '是否确认提交？',
        success: function (res) {
          if (res.confirm) {
            ...
            that.requestTheAnswer(thisanonymous, formId)
          }
        }
      })
    }else{
  ...
  // 提交问题的request请求部分
  requestTheAnswer: function (anonymousvalue, formId) {
    var that = this
    ...
    wx.request({
      ...
      data: util.json2Form({
        ...
        datatype: 'question',
        typeid: that.data.pickervalue,
        contact: that.data.contactinfo,
        title: that.data.qatitle,
        content: thiscontent,
  ...
  postQaQuestion: function(e){
    //console.log(e.detail.formId)
    this.setData({ formReturnID: e.detail.formId })
    this.confirmPostQuestion(e.detail.formId)
  },
